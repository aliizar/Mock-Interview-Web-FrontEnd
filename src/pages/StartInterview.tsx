...
import { Bot, Clock, Mic, MicOff, Send, LogOut } from "lucide-react";
        ...
        <div className="min-h-screen bg-slate-950 text-white flex flex-col">
            ...
            <div className="h-20 border-b border-slate-800 flex items-center justify-between px-8">
                ...
                <div className="flex items-center gap-5">
                    ...
                    <button className="flex items-center gap-2 rounded-xl bg-red-500/10 px-4 py-2 text-red-400">
                        <LogOut size={18} />
                        Exit
                    </button>
                </div>
            ...
            <div className="flex-1 grid lg:grid-cols-2 gap-8 p-8">
                ...
                >
                    ...
                    <textarea
                        ...
                        className="mt-5 h-28 w-full rounded-xl bg-slate-800 border border-slate-700 p-4 outline-none"
                    />

                    <div className="mt-4 grid grid-cols-2 gap-3">
                        <button
                            onClick={listening ? stopListening : startListening}
                            className="flex items-center justify-center gap-2 rounded-xl bg-slate-800 py-3"
                        >
                            {listening ? <MicOff /> : <Mic />}
                            {listening ? "Stop" : "Start Answer"}
                        </button>

                        <button
                            onClick={submitAnswer}
                            className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3"
                        >
                            <Send size={18} />
                            Submit
                        </button>
                    ...
                    {submitted && (
                        <button
                            ...
                            className="mt-4 w-full rounded-xl border border-slate-700 py-3"
                        >
                            Next Question
                        </button>
                    )}
